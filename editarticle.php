...
<html lang="zh-cn">
	...
	<body ondragstart="return false"  onselect="document.selection.empty()">
        ...
        if(!empty($_GET['id'])){
            $edit = $_GET['id'];
            $sql = "select * from artical where id='$edit'";
        ...
        if (!empty($_POST['sub'])) {    
            ...
            $hid = $_POST['hid'];
            $sql="update artical set title='$title', contents='$con' where id='$hid'";
            mysqli_query($link, $sql);  //执行插入语句 
            
            echo "<script>alert('博客修改成功！');location.href='message.php'</script>";
        }
        ?>
